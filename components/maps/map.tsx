...
export default function Map() {
  ...
  useEffect(() => {
    if (selectedPlace) setCurrentImageIndex(0);
  }, [selectedPlace]);
  ...
  const nextImage = useCallback(() => {
    ...
    setCurrentImageIndex((prev) =>
      prev === selectedPlace.placeImages.length - 1 ? 0 : prev + 1,
    );
  }, [selectedPlace]);
  ...
  const prevImage = () => {
    if (!selectedPlace || !selectedPlace.placeImages.length) return;
    setCurrentImageIndex((prev) =>
      prev === 0 ? selectedPlace.placeImages.length - 1 : prev - 1,
    );
  };
        ...
        <div className="absolute inset-0 z-[1000] flex items-center justify-center bg-black/50 p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto relative flex flex-col">
            <button
              ...
              className="absolute top-4 right-4 bg-white/80 p-2 rounded-full hover:bg-gray-100 transition z-20"
            >
            ...
            <div className="relative w-full h-64 sm:h-80 flex-shrink-0 bg-gray-900 group">
              ...
              {selectedPlace.placeImages &&
              selectedPlace.placeImages.length > 0 ? (
                <>
                    ...
                    <>
                      ...
                      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-10">
                        {selectedPlace.placeImages.map((_, idx) => (
                          <div
                            key={idx}
                            className={`h-2 w-2 rounded-full transition-all ${idx === currentImageIndex ? "bg-white w-6" : "bg-white/50"}`}
                          />
                        ))}
                      </div>
                    </>
            ...
            <div className="p-6 space-y-6">
              <div>
                ...
                <p className="text-gray-700 whitespace-pre-line">
                  {selectedPlace.description || "Tidak ada deskripsi."}
                </p>
              </div>
